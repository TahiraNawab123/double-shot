...
const menu = [
  { id: 'espresso', name: 'Espresso', description: 'A clean, syrupy double shot.', price: 280, category: 'Coffee' },
  { id: 'flat-white', name: 'Flat White', description: 'Velvety microfoam, double ristretto.', price: 520, category: 'Coffee' },
  { id: 'iced-mocha', name: 'Iced Mocha', description: 'Cocoa, espresso, milk, over ice.', price: 650, category: 'Cold' },
  { id: 'cardamom-bun', name: 'Cardamom Bun', description: 'Buttery layers, toasted sugar.', price: 480, category: 'Bakes' },
  { id: 'avocado-toast', name: 'Avocado Toast', description: 'Sourdough, lemon, za’atar, chilli.', price: 890, category: 'Plates' },
  { id: 'basque-cheesecake', name: 'Basque Cheesecake', description: 'Burnt top, soft centre, sea salt.', price: 720, category: 'Sweet' },
]
...
function Home({ onMenu, onReserve }: { onMenu: () => void; onReserve: () => void }) { return <><section className="hero"><div className="hero-copy"><p className="eyebrow">MODEL TOWN · LAHORE</p><h1>Good coffee.<br /><em>Better company.</em></h1><p className="lede">An all-day coffee house for slow mornings, long conversations, and the perfect second cup.</p><div className="button-row"><button className="primary-button" onClick={onMenu}>VIEW THE MENU <ArrowRight size={16} /></button><button className="secondary-button" onClick={onReserve}>RESERVE A TABLE</button></div></div><div className="hero-image"><Image src="/double-shot-hero.png" alt="Latte on a café table" fill priority sizes="(max-width: 768px) 100vw, 50vw" /></div></section><section className="intro-section"><p className="eyebrow">THE DOUBLE SHOT EDIT</p><h2>A little ritual<br /><em>worth repeating.</em></h2><p>From first pour to last bite, every detail is considered. Explore our drinks, settle into your favourite corner, and stay as long as you like.</p></section><section className="split-section"><div><p className="eyebrow">ON THE BAR</p><h2>Made for<br /><em>your mood.</em></h2><button className="text-link">EXPLORE EVERYTHING <ArrowRight size={14} /></button></div><div className="feature-card"><div className="feature-number">01</div><h3>The classics</h3><p>Espresso, flat whites and the quiet confidence of a well-pulled shot.</p></div><div className="feature-card dark-card"><div className="feature-number">02</div><h3>Something to eat</h3><p>Savoury plates, flaky layers and small sweet endings.</p></div></section><section className="visit-band"><p className="eyebrow">COME BY</p><h2>See you in<br /><em>Model Town.</em></h2><div className="visit-details"><span><MapPin size={16} /> C, Shop #7, C Block, Sector Model Town, Lahore</span><span><Clock3 size={16} /> Open daily · 6:00 am — 12:00 am</span><button className="text-link">+92 318 9119222 <ArrowRight size={14} /></button></div></section></> }

function MenuPage({ onAdd }: { onAdd: (id: string) => void }) { const [category, setCategory] = useState('All'); const filtered = category === 'All' ? menu : menu.filter((item) => item.category === category); return <section className="menu-page"><div className="page-heading"><p className="eyebrow">THE MENU · MODEL TOWN</p><h1>Good things,<br /><em>made daily.</em></h1><p>Small rituals, big comfort. Everything is prepared in-house and served with care.</p></div><div className="category-row">{['All', 'Coffee', 'Cold', 'Bakes', 'Plates', 'Sweet'].map((item) => <button key={item} className={category === item ? 'category active' : 'category'} onClick={() => setCategory(item)}>{item}</button>)}</div><div className="menu-grid">{filtered.map((item, index) => <article className="menu-item" key={item.id}><div className={`menu-art art-${index % 3}`}><Sparkles size={22} /></div><div className="menu-item-info"><div><p className="eyebrow">{item.category}</p><h3>{item.name}</h3><p>{item.description}</p></div><div className="menu-item-bottom"><strong>PKR {item.price.toLocaleString()}</strong><button className="add-button" onClick={() => onAdd(item.id)}><Plus size={16} /> ADD</button></div></div></article>)}</div></section> }
...
function Visit() { return <section className="visit-page"><p className="eyebrow">FIND US</p><h1>Come as<br /><em>you are.</em></h1><div className="map-placeholder"><MapPin size={28} /><span>C, Shop #7 · Model Town, Lahore</span></div><div className="visit-info"><strong>Open daily</strong><span>6:00 am — midnight</span><strong>Call us</strong><span>+92 318 9119222</span></div></section> }
